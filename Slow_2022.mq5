...
void DeleteAllOrdersByMagic(int magic_number_Delete)
  {
   ulong order_ticket_;
   //long const magic_number
//--- go through all pending orders
   for(int i=OrdersTotal()-1;i>=0;i--)
      if((order_ticket_=OrderGetTicket(i))>0)
         //--- order with appropriate ORDER_MAGIC
         if(magic_number_Delete==OrderGetInteger(ORDER_MAGIC))
           {
            MqlTradeResult result={};
            MqlTradeRequest request={};
            request.order=order_ticket_;
...
void RunBuy(string id_){
   if(Gap>=GapAllow&&TradeOn=="ON"&&GapsOn=="ON"&&SellBuy=="Buy"&&OrderIsOpend==0){
...
void RunSell(string id_){
   if(Gap>=GapAllow&&TradeOn=="ON"&&GapsOn=="ON"&&SellBuy=="Sell"&&OrderIsOpend==0){
